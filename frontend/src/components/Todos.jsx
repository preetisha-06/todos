...
const SERVER_URL =
  import.meta.env.MODE === "production"//production mode ho bhane vite server use garne otherwise aruma initialize garne
...
const Todos = () => {
  ...
  useEffect(() => {//function, dependancy array paramters linxa
    const fetchTodos = async () => {
      try {
        const response = await axios.get(`${SERVER_URL}/api/v1/todos`);//location ma request pathaunxa ra server le response pathaunxa client lai
  ...
  const handleCreateTodo = async (e) => {
    ...
    try {
      const response = await axios.post(//time consuming so response na aye samma code run gardeina tala ko
  ...
  const handleEditTodo = async (e) => {
    ...
    try {
      const response = await axios.patch(//kun ma patch request pathaune parameter ho
